...
ensemble_pred<-ifelse(rowMeans(ensemble)<=6/4,1,2)#There are more outpatients
...
accuracy<-data.frame(row.names = c("LDA","GLM","QDA","RPART","ENSEMBLE","ENSEMBLE2"),
                     Accuracy=c(lda_ac,glm_ac,qda_ac,rpart_ac,ensemble_ac,ensemble2_ac))
accuracy
acc_out<-data.frame(row.names = c("LDA","GLM","QDA","RPART","ENSEMBLE","ENSEMBLE2"),
                    Accuracy=c(lda_out,glm_out,qda_out,rpart_out,ensemble_out,ensemble2_out))
acc_out
acc_in<-data.frame(row.names = c("LDA","GLM","QDA","RPART","ENSEMBLE","ENSEMBLE2"),
                   Accuracy=c(lda_in,glm_in,qda_in,rpart_in,ensemble_in,ensemble2_in))
acc_in