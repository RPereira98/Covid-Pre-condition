...
Both sexes have more outpatients than inpatients,with  men having a slightly higher percentage of inpatients. The correlation of sex  and patient type is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$sex)
```
A small correlation value. An interpretation could be that sex doesn't influence much in the patient type.
...
More patients with pneumonia are inpatients than outpatients, and more patients without pneumonia are outpatients than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$pneumonia)
```
The correlation value is big and negative, indicating that patients with pneumonia are more susceptible to need hospitalization.
...
There are slightly more patients with diabetes that are inpatients than outpatients, and there are more patients without diabetes that are outpatients than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$diabetes)
```
A small negative value, maybe the diabetes influence a little in the patient type.
...
There are more patients with COPD that are inpatients than outpatients and there are more outpatients without COPD than inpatients without COPD. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$copd)
```
A very small negative correlation value, may be due to the small quantity of patients with COPD.
...
Both patients with and without asthma have more outpatients than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$asthma)
```
A very small positive value, maybe asthma doesn't influence much in the need of hospitalization or not.
...
There are slightly more patients with immunosuppression that are inpatient than outpatient and there are more outpatients without immunosuppression than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$asthma)
```
A very small negative value, maybe due to the small  quantity of patients with immunosuppression.
...
There are slightly more patients with hypertension that are inpatients than outpatients and more patients without hypertension that are outpatients than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$hypertension)
```
A small negative value, maybe hypertension doesn't influence a lot in the hospitalization.
...
There are slightly more outpatients with other diseases than inpatients and there are more outpatients without other diseases than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$other_disease)
```
A very small negative value, may be due to the small quantity of patients with other diseases.
...
There are slightly more inpatients with other cardiovascular diseases than outpatients and there are more outpatients without cardiovascular diseases than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$cardiovascular)
```
A very small negative value, may be due to the small quantity of patients with cardiovascular diseases.
...
There are slightly more outpatients with obesity than inpatients and there are more outpatients without obesity than inpatients. The correlation value is: 
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$cardiovascular)
```
A very small negative value, maybe obesity doesn't influence much in the type of patient.
...
There are more inpatients with chronic kidney disease than outpatients and more outpatients without chronic kidney disease than inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$renal_chronic)
```
A small negative value. The small value may be due to the small quantity of patients with the disease,but is bigger than other parameters with small quantities of patients with the disease, maybe influence considerably the type of patient.
...
With or without smoking habits the number of outpatients is bigger than the number of inpatients. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$tobacco)
```
A small negative value, maybe smoking habit doesn't influence a lot in the type of patient.
...
```
The proportion of inpatients change with age, it grows with the age. The correlation value is:
```{r echo=FALSE,warning=FALSE,message=FALSE}
cor(covid_dat$patient_type,covid_dat$age)
```
A  not very big but considerable positive value. Maybe age influence considerably in the patient type.
...
## Conclusion
 The analysis of the plots and correlation values are based on the sample. The negative correlation value indicates (except for sex and age) that a patient with the pre-condition is more susceptible to be hospitalized (inpatient), a positive value indicate that a patient with the pre-condition is more susceptible to be an outpatient. For sex the positive value indicate  that a male is more susceptible to be hospitalized, but the value is very small, so maybe doesn't influence much. For age the  positive value indicate that the older the patient the more susceptible to need a hospitalization is. For confirmation of these conclusions more studies are needed.
